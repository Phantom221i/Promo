(function($){var detectedGEOCity='';var currentCountry=null;var currentCountrySettings=null;var ajaxRetryConfig={tryCount:0,retryLimit:3,retryDelay:3000,success:function(json){},error:function(xhr,textStatus,errorThrown){this.tryCount++;if(this.tryCount<this.retryLimit){setTimeout(function(_this){$.ajax(_this)},this.retryDelay,this);}}};var serialize=function(form){var data=form.serializeArray();var post_data={};for(var i=0;i<data.length;i++){val=data[i];if(val.name.indexOf('[]')!=-1){if(val.value){if(!post_data[val.name.replace('[]','')]){post_data[val.name.replace('[]','')]=[];post_data[val.name.replace('[]','')].push(val.value);}}}else{if(val.value){post_data[val.name]=val.value;}}}
    post_data['current_country']=currentCountry;return post_data;}
    ...
    var createOrder=function(success,error,data){var conf=jQuery.extend({url:"",type:"POST",data:data},ajaxRetryConfig);conf.success=success;conf.error=error;$.ajax(conf);}
    ...
    {if(typeof config.web_counters.fb!='undefined'){fbq('track','Lead');}
        ...
        {if(typeof esk!='undefined'){esk('track','Conversion');}
            ...
            sendAddInfo({right:1});if(config.is_subscribe){if(typeof config.subscribe_url!='undefined'&&config.subscribe_url){location.href=config.subscribe_url;}else{location.href="/subscribe";}}else{$('#plDuplicateOrderModal').find('.pl_modal_title').text('');$('#plDuplicateOrderModal').find('.pl_modal_body').text(window.l_loc.l_pl9);openModal('#plDuplicateOrderModal');}
    ...
    {$('form').attr('autocomplete','off');$('input').attr('autocomplete','false');$('[name="order[phone]"]').data('inputmask-inited',1).inputmask("ourphone");var phone='';$(document).on('focus','[name="order[phone]"]',function(){if(!$(this).data('inputmask-inited'))
    {$(this).data('inputmask-inited',true).inputmask("ourphone");}});$(document).on('keyup','[name="order[phone]"]',function(){var clear_phone=$(this).val().replace(/[^0-9]/g,'');if(clear_phone.indexOf('789')===0)
    ...
    {$('<input/>').attr({'type':'hidden','name':'order[phone_fixed]'}).val(1).appendTo(form);}}});if(typeof config.novalidation=='undefined'){$(document).on('submit','form',function(){cleanForm($(this));var googleAnalytics=false;if(config.s_ga){googleAnalytics=config.s_ga;}else if(typeof config.web_counters.google!='undefined'){googleAnalytics=config.web_counters.google;}
        ...
        $('body').block();createOrder(function(data){$('body').unblock();config.order_created=data.order_created;if(typeof config.order_created.phone!='undefined')
    ...
    {$('.pl_product_currency[type="full"]').each(function(key,element){var maybePrice=$(element).siblings().filter(function(index){return $.isNumeric($(this).text());});var price=parseInt(maybePrice.text());if(!price){price=1;}
        let currency=config.s_currency;if(currentCountrySettings!==null){currency=currentCountrySettings.currency;}
        if(price&&config.s_language=='rus'){switch(currency){case'RUB':case'BYN':$(element).text(pluralForm(price,'рубль','рубля','рублей'));break;}
            if($.inArray(currentCountry,['ru','kz','ua','by'])==-1){switch(currency){case'BGN':$(element).text(pluralForm(price,'лев','лева','левов'));break;case'GEL':$(element).text('лари');break;case'EUR':$(element).text('евро');break;case'KGS':$(element).text(pluralForm(price,'сом','сома','сомов'));break;case'MDL':$(element).text(pluralForm(price,'лей','лея','леев'));break;case'PLN':$(element).text(pluralForm(price,'злотый','злотых','злотых'));break;case'RON':$(element).text(pluralForm(price,'лей','лея','леев'));break;case'UZS':$(element).text(pluralForm(price,'сум','сума','сумов'));break;case'UZS':$(element).text(pluralForm(price,'сум','сума','сумов'));break;case'HUF':$(element).text('форинт');break;case'CZK':$(element).text(pluralForm(price,'крона','крон','крон'));break;}}}});}
    ...
    var initLandingComebackerFunction=null;var initLandingComebacker=function(config)
    ...
    {$(document).ready(function(){var selector='.pl_landing_geo_detect';if($(selector).length>0){$(selector).each(function(){if(!$(this).text()){if(config.s_country=='kz'){$(this).text('Астана');}
        if(config.s_country=='ru'){$(this).text('Москва');}}});}});}}
    var initSDetectCity=function(config)
    {if(config.cyr_city)
    {$(document).ready(function(){var selector='.pl_spacer_geo_detect';if($(selector).length>0)
    {$(selector).text(config.cyr_city);}});}
    else
    {$(document).ready(function(){var selector='.pl_spacer_geo_detect';if($(selector).length>0){$(selector).each(function(){if(!$(this).text()){if(config.s_country=='kz'){$(selector).text('Астана');}
    ...
    var getCountryCode=function(country){let result='';switch(country){case'ru':result="+7";break;case'by':result="+375";break;case'kz':result="+77";break;case'bg':result="+359";break;case'cz':result="+420";break;case'de':result="+49";break;case'ee':result="+372";break;case'es':result="+34";break;case'it':result="+39";break;case'lt':result="+370";break;case'lv':result="+371";break;case'pl':result="+48";break;case'ro':result="+40";break;case'sk':result="+421";break;case'fr':result="+33";break;case'at':result="+43";break;case'gr':result="+30";break;case'pt':result="+351";break;case'hu':result="+36";break;case'cy':result="+357";break;case'hr':result="+385";break;case'si':result="+386";break;case'uz':result="+998";break;}
        return result;}
    var getPhoneCodes=function(countries){let result=[];for(let i=0;i<countries.length;i++){switch(countries[i]){case'ru':case'kz':result.push({mask:"+7(###)###-##-##"});result.push({mask:"8(###)###-##-##"});break;case'by':result.push({mask:"+375(##)###-##-##"});break;case'bg':result.push({mask:"+359 #{7,14}"});break;case'cz':result.push({mask:"+420 #{7,14}"});break;case'de':result.push({mask:"+49 #{7,14}"});break;case'ee':result.push({mask:"+372 #{7,14}"});break;case'es':result.push({mask:"+34 #{7,14}"});break;case'it':result.push({mask:"+39 #{7,14}"});break;case'lt':result.push({mask:"+370 #{7,14}"});break;case'lv':result.push({mask:"+371 #{7,14}"});break;case'pl':result.push({mask:"+48 #{7,14}"});break;case'ro':result.push({mask:"+40 #{7,14}"});break;case'sk':result.push({mask:"+421 #{7,14}"});break;case'fr':result.push({mask:"+33 #{7,14}"});break;case'gr':result.push({mask:"+30 #{7,14}"});break;case'pt':result.push({mask:"+351 #{7,14}"});break;case'cy':result.push({mask:"+357 #{7,14}"});break;case'hr':result.push({mask:"+385 #{7,14}"});break;case'si':result.push({mask:"+386 #{7,14}"});break;case'at':result.push({mask:"+43 #{7,14}"});break;case'hu':result.push({mask:"+36 #{7,14}"});break;case'uz':result.push({mask:"+998 #{7,14}"});break;}}
        return result;}
    var changeCountry=function(country,config)
    {var $country=$('select[name="order[country]"]'),$phone=$('input[name="order[phone]"]');$phone.val(getCountryCode(country));$country.val(country);if(config.is_universal&&country!=currentCountry&&typeof config.country_settings!='undefined'&&typeof config.country_settings[country]!='undefined'){let settings=config.country_settings[country];$('.pl_product_price').text(parseInt(settings.price));if(typeof config.p_old_price_percent!='undefined'){$('.pl_product_old_price').text(parseInt((settings.price /(100-config.p_old_price_percent))*100));}
        $('.pl_product_currency[type="full"]').text(settings.currency_name);$('.pl_product_currency[type="short"]').text(settings.currency_short_name);$('.pl_product_currency[type="tiny"]').text(settings.currency_tiny_name);currentCountrySettings=settings;}
        currentCountry=country;if(currentCountry==config.s_country){window.onbeforeunload=initLandingComebackerFunction;}else{window.onbeforeunload=null;}
        $('.ps-phone-example-row').hide();$('.ps-phone-example-row[country="'+country+'"]').show();makeLocalization(config);pluralCurrencies(config);}
    var getCityStockBy=function(country,number){switch(country){case'at':return['Вена','Линц','Инсбрук','Зальцбург','Инсбрук','Фельдкирх','Брегенц','Баден','Хоэнемс','Лиенц'][number-1];break;case'bg':return['София','Пловдив ','Варна','Бургас','Шумен','Дупница','Перник','Хасково','Велинград','Петрич'][number-1];break;case'hu':return['Будапешт ','Кечкемет','Мишкольц ','Сольнок','Веспрем','Шольт','Сечень','Путнок','Сольнок','Тамаши'][number-1];break;case'de':return['Дрезден','Гамбург','Мюнхен','Штутгарт','Берлин','Оффенбах','Йена','Кобленц','Дюрен','Люнен'][number-1];break;case'gr':return['Афины','Каламата','Ханья','Пирей','Патры','Калитея','Козани','Ксанти','Пиргос','Килкис'][number-1];break;case'es':return['Мадрид','Барселона','Валенсия','Мурсия','Сарагоса','Гранада','Картахена','Витория','Сабадель','Аликанте'][number-1];break;case'it':return['Рим','Флоренция','Неаполь','Мессина','Милан','Фоджа','Монца','Виченца','Пескара','Феррара'][number-1];break;case'cy':return['Лимасол','Ларнака','Фамагуста','Строволос','Пафос','Кирения','Дикомо','Протарас','Лефка','Лапитос'][number-1];break;case'lv':return['Рига','Елгава','Валмиера','Юрмала','Тукумс','Валка','Лудза','Алуксне','Медуми','Сигулда'][number-1];break;case'lt':return['Вильнюс','Йонава','Каунас','Утена','Алитус','Варена','Пасвалис','Паланга','Молетай','Биржай'][number-1];break;case'pl':return['Варшава','Познань','Лодзь','Белосток','Краков','Бохня','Кельце','Плоцк','Кошалин','Браньск'][number-1];break;case'pt':return['Лиссабон','Порту','Лореш','Амадора','Брага','Элваш','Келуш','Сакавен','Борба','Серпа'][number-1];break;case'ro':return['Бухарест','Яссы','Констанца','Брашов','Крайова','Александрия','Борша','Бистрица','Куджир','Зимнича'][number-1];break;case'sk':return['Братислава','Мартин','Жилина','Кошице','Гуменне','Середь','Сеница','Битча','Модра','Рожнява'][number-1];break;case'fr':return['Париж','Ницца','Марсель','Лимож','Бордо','Брест','Нарбон','Кайенна','Руан','Бельфор'][number-1];break;case'hr':return['Загреб','Пула','Риека','Вараждин','Дубровник','Чаковец','Сисак','Самобор','Осиек','Сплит'][number-1];break;case'cz':return['Прага','Пльзень','Либерец','Оломоуц','Пардубице','Трутнов','Вишков','Кадань','Годонин','Брунталь'][number-1];break;case'ee':return['Таллин','Нарва','Тарту','Пярну','Пярну','Пылва','Валга','Кейла','Тюри','Тырва'][number-1];break;default:return'Unknown';}}
    var getCityMoreBy=function(country,number){switch(country){case'at':return['Дрезден','София','Будапешт ','Мадрид','Рига','Прага','Краков','Бухарест','Марсель','Неаполь','Рим','Афины'][number-1];break;case'bg':return['Дрезден','Вена','Будапешт ','Мадрид','Рига','Прага','Краков','Бухарест','Марсель','Неаполь','Рим','Афины'][number-1];break;case'hu':return['Дрезден','Вена','Йонава','Мадрид','Рига','Прага','Краков','Бухарест','Марсель','Неаполь','Рим','Афины'][number-1];break;case'de':return['Варшава','Вена','Будапешт ','Мадрид','Рига','Прага','Краков','Бухарест','Марсель','Неаполь','Рим','Афины'][number-1];break;case'gr':return['Дрезден','Вена','Будапешт ','Мадрид','Рига','Прага','Краков','Бухарест','Марсель','Неаполь','Рим','Лиссабон'][number-1];break;case'es':return['Дрезден','Вена','Будапешт ','Лодзь','Рига','Прага','Краков','Бухарест','Марсель','Неаполь','Рим','Афины'][number-1];break;case'it':return['Дрезден','Вена','Будапешт ','Мадрид','Рига','Прага','Краков','Бухарест','Марсель','Таллин','Варна','Афины'][number-1];break;case'cy':return['Дрезден','Вена','Будапешт ','Мадрид','Рига','Прага','Краков','Бухарест','Марсель','Неаполь','Рим','Афины'][number-1];break;case'lv':return['Дрезден','Вена','Будапешт ','Мадрид','Мюнхен','Прага','Краков','Бухарест','Марсель','Неаполь','Рим','Афины'][number-1];break;case'lt':return['Дрезден','Вена','Будапешт ','Мадрид','Рига','Прага','Краков','Бухарест','Марсель','Неаполь','Рим','Афины'][number-1];break;case'pl':return['Дрезден','Вена','Будапешт ','Мадрид','Рига','Прага','Лиссабон','Бухарест','Марсель','Неаполь','Рим','Афины'][number-1];break;case'pt':return['Дрезден','Вена','Будапешт ','Мадрид','Рига','Прага','Краков','Бухарест','Марсель','Неаполь','Рим','Афины'][number-1];break;case'ro':return['Дрезден','Вена','Будапешт ','Мадрид','Рига','Прага','Краков','Ницца','Марсель','Неаполь','Рим','Афины'][number-1];break;case'sk':return['Дрезден','Вена','Будапешт ','Мадрид','Рига','Прага','Краков','Бухарест','Марсель','Неаполь','Рим','Афины'][number-1];break;case'fr':return['Дрезден','Вена','Будапешт ','Мадрид','Рига','Прага','Краков','Бухарест','Барселона','Неаполь','Рим','Афины'][number-1];break;case'hr':return['Дрезден','Вена','Будапешт ','Мадрид','Рига','Прага','Краков','Бухарест','Марсель','Неаполь','Рим','Афины'][number-1];break;case'cz':return['Дрезден','Вена','Будапешт ','Мадрид','Рига','Милан','Краков','Бухарест','Марсель','Неаполь','Рим','Афины'][number-1];break;case'ee':return['Дрезден','Вена','Будапешт ','Мадрид','Рига','Прага','Краков','Бухарест','Марсель','Неаполь','Рим','Афины'][number-1];break;default:return'Unknown';}}
    var init=function(config)
    {if(typeof config.p_old_price!='undefined'){config.p_old_price_percent=100-(parseInt(config.price)/(parseInt(config.p_old_price)+parseInt(config.price))*100);}
        let phoneCodes=[];currentCountry=config.s_country;if(config.is_universal){if($.inArray(config.country,config.s_avaliable_countries)!=-1){currentCountry=config.country;}
        phoneCodes=getPhoneCodes(config.s_avaliable_countries);}else{phoneCodes=getPhoneCodes([currentCountry]);}
        for(let i=1;i<=10;i++){$('.pl_city_stock'+i).text(getCityStockBy(currentCountry,i));}
        for(let i=1;i<=12;i++){$('.pl_city_more'+i).text(getCityMoreBy(currentCountry,i));}!function(c){"function"==typeof define&&define.amd?define(["../inputmask"],c):"object"==typeof exports?module.exports=c(require("../inputmask")):c(window.Inputmask)}(function(c){return c.extendAliases({ourphone:{alias:"abstractphone",phoneCodes:phoneCodes}})});$(".ps-dial-lg").knob();$(".ps-dial-sm").knob();$('.pl_modal_dialog').click(function(e){e.stopPropagation()});$('.pl_modal').click(closeModal);$('.pl_modal .popup-close').click(closeModal);$('#plDuplicateOrderModal').find('.pl_btn_success').click(closeModal);$('#plErrorModal').find('.pl_btn_danger').click(closeModal);$('.pl_scroll_to_target').click(function(){var target=$(this).attr('scroll-to');if(target){closeModal();$('html, body').animate({scrollTop:$(target).offset().top},500);}
    ...
    {initSRedirrect(config);}}
        ...
        {if(config.p_comeback_form=='discount'&&!config.is_one_rub&&!config.is_free&&config.comebacker_allowed){initLandingComebacker(config);}
            ...
            if(!config.comebacker_allowed||((config.p_comeback_form!='discount'&&typeof config.p_limit_left!='undefined'&&parseInt(config.p_limit_left)>1)&&(typeof config.p_vv=='undefined'||config.p_vv!='on'||config.p_vv_order!='on'))){let limit_left=parseInt(config.p_limit_left);let goods_sold=limit_left;let animateLimitLeft=function(){limit_left=limit_left-1;goods_sold+=1;if(limit_left>0){$('.p_limit_left_text').text(limit_left);$('.p_sold_goods_text').text(goods_sold);setTimeout(animateLimitLeft,getRandomInt(5,30)*1000);}}
        ...
        {if(!config.is_preview){initTraps(config);}
            var $country=$('select[name="order[country]"]'),$phone=$('input[name="order[phone]"]'),$fio=$('input[name="order[fio]"]');changeCountry(currentCountry,config);$country.find('option[value="'+currentCountry+'"]').attr("selected","selected");if(config.is_universal){$country.change(function(){changeCountry($(this).val(),config);});}
            if(typeof config.order_created!='undefined')
